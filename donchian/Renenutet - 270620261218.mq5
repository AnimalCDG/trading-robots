...
double            max3_7                  = 0.0;
bool              block3_7                = false;
int               InpMagicShort           = 0307; /* Número mágico */
...
void OnTick()
{
   ...
   if( IsOrderOpen(InpMagicShort) == false && hasMaximumBroken3_7 == true && (askShortLast != max3_7))
   {
      ...
      //Atualizar o stop
      //ApplyTrailingStop(0, InpMagicShort, 0);
      GerenciarStops(InpMagicShort);
   }
...
//+------------------------------------------+
void ApplyTrailingStop(int targetPeriod, int numberMagic, double newStopLoss)
{   
...
void GerenciarStops(long magic)
{
   PosicaoTrade posicoes[];
   int total = 0;

   // Obtém todas as posições do Magic
   for(int i = PositionsTotal()-1; i >= 0; i--)
   {
      ...
      ArrayResize(posicoes, total + 1);

      posicoes[total].ticket        = ticket;
      posicoes[total].precoEntrada  = PositionGetDouble(POSITION_PRICE_OPEN);
      posicoes[total].stopLoss      = PositionGetDouble(POSITION_SL);
      posicoes[total].takeProfit    = PositionGetDouble(POSITION_TP);
   ...
   // Ordena pelo Take Profit
   OrdenarPorTP(posicoes);
   ...
   {
      // TP1 já ocorreu
      novoSL = posicoes[0].precoEntrada * (1.0 + 0.00033);
   }
   else if(total == 1)
   {
      // TP1 e TP2 ocorreram
      novoSL = posicoes[0].precoEntrada * (1.0 + 0.00066);
   }

   AtualizarStops(posicoes, novoSL);
}

void OrdenarPorTP(PosicaoTrade &vetor[])
{
   int total = ArraySize(vetor);

   for(int i=0;i<total-1;i++)
   {
      for(int j=i+1;j<total;j++)
      {
         if(vetor[i].takeProfit > vetor[j].takeProfit)
         {
            PosicaoTrade aux = vetor[i];
            vetor[i] = vetor[j];
            vetor[j] = aux;
         }
      }
...
void AtualizarStops(PosicaoTrade &vetor[], double novoSL)
{
   for(int i=0;i<ArraySize(vetor);i++)
   {
      if(!PositionSelectByTicket(vetor[i].ticket))
         continue;

      double slAtual = PositionGetDouble(POSITION_SL);
      double tpAtual = PositionGetDouble(POSITION_TP);

      // Evita enviar alteração desnecessária
      if(slAtual >= novoSL)
         continue;

      trade.PositionModify(
            vetor[i].ticket,
            NormalizeDouble(novoSL,_Digits),
            tpAtual
      );
   }
}